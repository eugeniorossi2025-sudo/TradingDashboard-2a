...
{
    public ProactiveEngine()
    {
        _rows = new Dictionary<int, RowState>();
        _lastAdvice = new Dictionary<int, Advice>();
        _tableMarginsUnits = new Dictionary<int, decimal>();
        _seenInputs = new HashSet<string>();
    }
    ...
    // 🔹 CORE DELLA SCUDERIA
    // ============================================================
    private readonly ScuderiaExtensions _scuderia = new();
    private readonly ProactiveSettings _s;
    private readonly RegiaAdaptiva _regiaDynamic;

    private readonly Dictionary<int, RowState> _rows;
    private readonly Dictionary<int, Advice> _lastAdvice;
    private readonly Dictionary<int, decimal> _tableMarginsUnits;
    private readonly HashSet<string> _seenInputs;
    ...
    private readonly Dictionary<int, LastInput> _lastInputs = new();
    ...
    private Advice CheckPreemptiveStopL5(int tableId)
    {
        ...
        var capResidua = EstimateResidualCapacityUnits();
        var triggerDebt = _portfolioDebtUnits > _s.DebtTriggerRatio * capResidua;
        var roomClosed = _heavyCount >= hmax || _cooldown > 0;
    ...
    public class StateData
    {
        public StateData()
        {
            Rows = new Dictionary<int, RowState>();
            SeenInputs = new List<string>();
            LastAdvice = new Dictionary<int, Advice>();
        }
        ...
        public Dictionary<int, RowState> Rows { get; set; }
        public List<string> SeenInputs { get; set; }
        public Dictionary<int, Advice> LastAdvice { get; set; }
    ...
    {
        var activeTables = Math.Max(1, _rows.Count);
        ...
        char esito, double totalElapsedMinutes, int totaltables)
    {
        ...
        // ============================================================
// ============================================================
// 🔥 PATCH DUPLICATI COMPLETA (W10 + HistoryTable + SignalW10)
// ============================================================
        var key = $"{tableId}-{handIndexMazzo}";
        var keySeen = _seenInputs.Contains(key);

        if (keySeen &&
            _lastAdvice.TryGetValue(tableId, out var lastAdv) &&
            _lastInputs.TryGetValue(tableId, out var lastIn))
        {
            var sameHand = lastIn.HandIndex == handIndexMazzo;
            var sameMargine = lastIn.MargineK == margineK;
            var sameMartingala = lastIn.MartingalaUi == martingalaUi;
            var sameEsito = lastIn.Esito == esito;

            if (sameHand && sameMargine && sameMartingala && sameEsito)
            {
                // 🔥 PATCH W10 – aggiorno sempre History anche nei duplicati
                if (esito != 'T')
                {
                ...
                // 🔥 PATCH SignalW10
                lastAdv.SignalW10 = IsSevereRedTableSignal(rsTmp) ? "Red" : "Green";

                // 🔁 DUPLICATO REALE → riuso
                return lastAdv;
            }
        }
        else
        ...
        // ============================================================
        rsTmp.WarmInputs++;
        var inGrace = rsTmp.WarmInputs <= 3;
        ...
        // ============================================================
        var k = (decimal)Math.Max(0.0000001, _s.K);
        var margineUnits = margineK / k;
        ...
        var levelIdx = OutcomeInferer.ToLevelIndex(martingalaUi);
        var stakeUnitsNow = _s.Levels[Math.Min(levelIdx, _s.Levels.Length - 1)];
        var stakeShownK = stakeUnitsNow * _s.K;
        ...
        var exitingHeavy = outcome == 'B' && rs.PrevLevel >= 5 && levelIdx == 0;
        ...
        var inHot = adv.HotZone;
        var severeRed = IsSevereRedTableSignal(rs);
        var hmaxClosed = _heavyCount >= regia.hmax;
        var roomCooldown = _cooldown > 0;
        ...
        if (levelIdx == 4 && !rs.ForceToL8Active)
        {
            var capResidua = EstimateResidualCapacityUnits();
            var triggerDebt = _portfolioDebtUnits > _s.DebtTriggerRatio * capResidua;
            var canOverride = _hotOverridesActive < _s.MaxHotOverridesConcurrent
