...
Partial Class PCManagement
    ...
    Dim SqlGrid As String = "Ups_PC_BO"
    Dim TableName As String = "dbo.PC"
    Dim _PageTitle As String = "PC Management"
    ...
    End Sub
    Protected Sub LnkDeleteJSON_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles LnkDeleteJSON.Click
        Dim oCon As SqlClient.SqlConnection

        'If IO.File.Exists(Server.MapPath("~/Repository/proactive_state.json")) Then
        'IO.File.Delete(Server.MapPath("~/Repository/proactive_state.json"))
        'End If
    ...
    Protected Sub RadGrid1_ItemCommand(ByVal source As Object, ByVal e As Telerik.Web.UI.GridCommandEventArgs) Handles RadGrid1.ItemCommand
        ...
        If (e.CommandName.ToString().ToLower() = "delete") Then
            ExecuteSQL("Delete from " & TableName & " where id=" & e.CommandArgument)

            RadGrid1.Rebind()
        End If

        If (e.CommandName.ToString().ToLower() = "clean") Then
            Dim oCon As SqlConnection

            oCon = Database.GetConn()

            Using cmd As New SqlCommand("UpD_PC", oCon) ' Nome della tua Stored Procedure
                cmd.CommandType = CommandType.StoredProcedure
                SqlCommandBuilder.DeriveParameters(cmd)
                cmd.Parameters("@Key").Value = Long.Parse(DateTime.Now.ToString("yyyyMMddHHmmssfff"))
                cmd.Parameters("@ID_PC").Value = e.CommandArgument
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        If Not (IsPostBack) Then
            If Utility.VerInt32(RetUserInfo("ID")) = 0 Or Utility.VerBool(RetUserInfo("Administrator")) = False Then
