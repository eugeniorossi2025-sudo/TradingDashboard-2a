...
namespace Gamebot.Models.UI
{
    internal class WorkerTask
    {
            ...
            if (!(areaScreen == "AREA_SALDO") || area != null)
            {
                ...
                while (Runtime.runningOCRScan && !ct.IsCancellationRequested)
                {
                    lock (lockWorker)
                    {
                        ...
                        {
                            currentImage = Monitor.Instance.IncreaseResolution(currentImage, 3);
                        }
                        ...
                        if (areaScreen.Equals("AREA_PUNTARE"))
                        {
                            ...
                            if (ocrResponse2.GetResponse().SuccessScan)
                            {
                                ...
                                OCReads.label_bet_last_success_update = DateTime.Now;
                            }
                        }
                        if (areaScreen.Equals("AREA_MAZZO"))
                        {
                            ...
                            OCRResponse ocrResponse3 = ocrScan.GetTextFromBitmapNumberDeckDefault(currentImage, false, "");
                            //OCReads.number_deck = -1;
        ...
        private void TaskStateMachine(CancellationToken ct)
        {
            ...
            var last_deck = -1;
            var last_deck_update = DateTime.Now;
            ...
            while (Runtime.runningStateMachineBot)
            {
                ...
                last_deck = OCReads.number_deck;
                last_deck_update = DateTime.Now;
                
                MainStateBot.StateMachine();
                
                /*
                while (last_deck == OCReads.number_deck && DateTime.Now - last_deck_update < TimeSpan.FromSeconds(1) && !ct.IsCancellationRequested);
                if (last_deck != OCReads.number_deck)
                {
                    temp = DateTime.Now;
                    while (DateTime.Now - temp < TimeSpan.FromSeconds(1) && !ct.IsCancellationRequested);

                    if (OCReads.number_deck >= 60)
                    {
                        temp = DateTime.Now;
                        while (DateTime.Now - temp < TimeSpan.FromSeconds(3) && !ct.IsCancellationRequested);
                    }
                }
                
                if (OCReads.number_deck == -1)
                {
                    temp = DateTime.Now;
                    while (DateTime.Now - temp < TimeSpan.FromSeconds(2) && !ct.IsCancellationRequested);
                } else if (OCReads.number_deck >= 60)
                {
                    temp = DateTime.Now;
                    while (DateTime.Now - temp < TimeSpan.FromSeconds(3) && !ct.IsCancellationRequested);
                }
