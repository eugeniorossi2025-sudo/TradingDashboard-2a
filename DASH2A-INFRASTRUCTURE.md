...
> **Ultimo aggiornamento: 2026-05-25** (stato validato in rete, vedi §11)
...
| HTTPS | Porta 443 non attiva — solo HTTP |
| Smoke test | `GET http://51.83.159.175/api/Auth/test` → **200** |
...
| API target (prod) | `http://51.83.159.175` |
...
| URL produzione | `http://51.83.159.175` |
...
| `firebase-hosting-merge.yml` | push `main` | Deploy frontend Firebase |
...
| Secret | Uso |
|---|---|
| `DASH2A_RDP_PASSWORD` | RDP emergenza |
| `FIREBASE_SERVICE_ACCOUNT_*` | Deploy Firebase |
| Connection string / JWT / SMTP | Override server o secrets — non in repo |

### Procedura deploy backend

```text
1. git push origin main (solo quando autorizzato)
2. GitHub Actions → deploy-backend-dash2a.yml → Run workflow
3. Input: I_UNDERSTAND_BACKEND_DEPLOY_ONLY
4. Smoke: http://51.83.159.175/api/Auth/test → 200
5. Login UI: https://eugenio-dashboard-2a.web.app/
```
...
| Firebase | account progetto | Console Firebase `eugenio-dashboard-2` |
...
| 6 | Frontend prod Firebase | **OK** | HTTP 200 |
